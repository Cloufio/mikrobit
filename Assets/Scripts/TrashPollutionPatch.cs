...
/// <summary>
/// A lightweight, procedural patch of murky water that sits beneath one trash
/// object. Cleaning changes the patch from brown to blue before it fades away.
/// </summary>
    ...
    private static Sprite GetPollutionSprite()
    {
        ...
        for (int y = 0; y < TextureHeight; y++)
        {
            for (int x = 0; x < TextureWidth; x++)
            {
                ...
                if (distance <= irregularEdge)
                {
                    float alpha = noise > 0.84f ? 0.42f : 1f;
